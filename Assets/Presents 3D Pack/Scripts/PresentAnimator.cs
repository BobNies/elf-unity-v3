...
public class PresentAnimator : MonoBehaviour
{
    ...
    public void LoadContainedItems()
    {
        PreloadGameObjects(ContainedItems, ref containedItemsPool);
    }
    ...
    void ActivateGameObjects(GameObject[] gameObjects, ref GameObject[] gameObjectsPool)
    {
        ...
        PreloadGameObjects(gameObjects, ref gameObjectsPool);
    ...
    {
        Debug.Log("bob DisablePrefabs:");
        // ContainedItems containedItemsPool
        if (containedItemsPool == null || containedItemsPool.Length == 0) return;

        for (int i =0; i < containedItemsPool.Length; i++)
        {
            if (containedItemsPool[i] != null)
            {
                            Debug.Log("bob destroy:");
                            BcpLogger.Trace("bob destroy"); 
                containedItemsPool[i].SetActive(false);
                Destroy(containedItemsPool[i]);
                containedItemsPool[i] = null;
            }
        }

        //containedItemsPool.Clear();
    }
